...
def lambda_handler(event, context):
    try:
        def not_equals(first, second):
            if (isinstance(first, numbers.Number) and
                    isinstance(second, numbers.Number)):
        ...
        for index, arg in enumerate(args):
            if not 'desafio{0}'.format(ndes) in locals():
                return (
                    "Nome da função inválido. Usar 'def desafio{0}(...)'"
                    .format(ndes))
...
def get_user_quiz(userid, quizid):
    ...
    cursor.execute("SELECT sent,answer,result from USERQUIZ where userid = '{0}' \
                    and quizid = {1} order by sent desc"
...
def get_quiz(quiz_id, user):
    ...
    if user == 'admin' or user == 'fabioja':
        cursor.execute("SELECT id, \
                        release, expire, problem, tests, results, diagnosis,\
                        numb from QUIZ where id = {0}".format(quiz_id))
    else:
        cursor.execute("SELECT id, \
                        release, expire, problem, tests, results, diagnosis,\
                        numb from QUIZ where id = {0} and release < '{1}'"
                       .format(quiz_id, datetime.now().strftime
                               ("%Y-%m-%d %H:%M:%S")))
    info = [reg for reg in cursor.fetchall()]
    conn.close()
    return info
...
def get_info(user):
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    cursor.execute("SELECT pass, type from USER where user = '{0}'"
                   .format(user))
    print("SELECT pass, type from USER where user = '{0}'"
          .format(user))
...
def main():
    ...
    if request.method == 'POST' and 'ID' in request.args:
        ...
        if len(quiz) == 0:
            ...
            return render_template('index.html', username=AUTH.username(),
                                   challenges=challenges, p=variable_p,
                                   msg=msg)
        ...
        with open(filename, 'r') as file_pointer:
            answer = file_pointer.read()

        # lamb = boto3.client('lambda')
        args = {"ndes": variable_id, "code": answer, "args": eval(quiz[4]),
    ...
    if len(challenges) == 0:
        ...
        variable_p = 2
        return render_template('index.html', username=AUTH.username(),
                               challenges=challenges, p=variable_p, msg=msg)
    ...
    answers = get_user_quiz(AUTH.username(), variable_id)
...
def change():
    if request.method == 'POST':
        ...
        if nova != repet:
            ...
            variable_p = 3
        elif (get_info(AUTH.username()) !=
              hashlib.md5(velha.encode()).hexdigest()):
